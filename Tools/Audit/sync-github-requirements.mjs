...
const R = [
  ...
  requirement('pwm-lighting-rgb-strip', 2, 'Complete PWM ownership, enclosure fade, status RGB, power light, and addressable strip behavior', 'open',
    ['🧩 firmware', '🎛️ front-panel', '🐛 regression', '🔍 needs-hardware'], 'PWM, enclosure light, power, RGB, and addressable LEDs', [
      ...
      'Keep status ownership explicit: native-capable firmware renders safety and normal states from its persistent EEPROM profile table, while the file-watched host configuration edits/provisions profiles, drives explicit bounded previews, and supplies the legacy RGB fallback only to boards without the native capabilities.',
      ...
      'Keep semantic Running/Idle claims host-owned rather than inferred from the reed, but let native-capable firmware render their physical status profiles and smoothly restore the underlying board-owned state after transient RF/door/BT cues.',
    ...
    ], 'Direct per-channel PWM, output persistence, and enclosure easing are source-tested. Status rendering is being converged on one native-capable firmware owner with a persistent per-condition EEPROM profile table, explicit host preview ownership, and a bounded legacy-host fallback; it is not a continuous host override on capable boards. Final cross-surface ownership tests and exact-current-firmware visual/load validation remain open for channel 0, reed-driven illumination, BT/RF/door priority, power/status outputs, and D6 pixel order.'),
  ...
  requirement('tui-pages-controls', 7, 'Build polished multipage TUI controls for board, settings, RF, programming, and automation', 'open',
    ['🖥️ host', '✨ ux', '⚡ priority: high', '💡 enhancement', '🚧 in progress'], 'TUI structure and interaction', [
      ...
      'Use modal/dialog editors by default, with slider plus typed 0..100% values/units for brightness and outputs; fix stale/off-by-one setting activation, provide default-page selection from Board and Menus, group precision fields, and edit every native status profile independently with a faithful bounded preview while clearly distinguishing persistent board assignments from host fallback configuration.',
      'Refresh Board values on page entry; hide build-only Swap temperature roles; group unrelated board/settings fields, show BT Audio only, retain a hidden TODO for future Bluetooth Serial, and expose concise user-facing status-owner labels without leaking raw scheduler mechanics; use HOST in user-facing text.',
