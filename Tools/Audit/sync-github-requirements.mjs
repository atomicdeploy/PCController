...
const EXTRA_ISSUE_REQUIRED_LABELS = {
  ...
  208: ['🧩 firmware', '🖥️ host', '🔌 protocol-api', '🧪 testing', '📚 documentation'],
  209: ['🧩 firmware', '🖥️ host', '🔌 protocol-api', '🧪 testing'],
  216: ['🧩 firmware', '🖥️ host', '🔌 protocol-api', '💾 storage', '🚀 programming', '🧪 testing', '🏗️ tooling-build'],
  217: ['🖥️ host', '🔌 protocol-api', '🧪 testing', '🌐 networking'],
  221: ['🧩 firmware', '🖥️ host', '🧪 testing', '📚 documentation', '🏗️ tooling-build'],
  227: ['🧩 firmware', '🖥️ host', '🔌 protocol-api', '🧪 testing', '🏗️ tooling-build'],
  235: ['🖥️ host', '🔌 protocol-api', '🧪 testing', '🌐 networking'],
  244: ['🖥️ host', '🔌 protocol-api', '🧪 testing', '✨ ux'],
  248: ['🧪 testing', '🏗️ tooling-build', '🔒 security'],
...
const SESSION = {
  ...
  current: 'direct-user-request-2026-08-13',
  issueLedger: 'github-issue-ledger-2026-08-13',
};
...
const PROMPT_EXCERPTS = {
  ...
  permissionAvailability: prompt(SESSION.current, 1, 'Never render capability-specific keys or values before the peer advertises the capability and the authoritative fetch is in progress or complete. Unknown is a real loading state; unavailable means the element is absent. Future permissions must also gate availability without re-enabling current alpha authentication.'),
  removeAlphaSchemaBaggage: prompt(SESSION.issueLedger, 208, 'Remove obsolete alpha schema revisions and compatibility branches; preserve history in Git and add a supported schema contract only for a real beta or release candidate.'),
  compactCapabilityDiscovery: prompt(SESSION.issueLedger, 209, 'Define a compact authoritative board capability discovery contract reusable by firmware, host parsers, offline tooling, generated contracts, and every control surface.'),
  uartFirstProgramming: prompt(SESSION.issueLedger, 216, 'Use UART and the normal Arduino or Urboot path first; use USBasp only for initialization or recovery, choose EEPROM backup transport by actual capability, deduplicate backups, and remove the incomplete-backup escape hatch.'),
  discoveryResponderCoexistence: prompt(SESSION.issueLedger, 217, 'Validate WS-Discovery and NetBIOS responder behavior on Windows where standard ports may already be owned, without overstating probe-only evidence.'),
  ownershipRoots: prompt(SESSION.issueLedger, 221, 'Give firmware, host, tooling, simulator, and documentation explicit contributor-friendly ownership roots, using history-preserving moves and one canonical build contract.'),
  virtualBoardAdapters: prompt(SESSION.issueLedger, 227, 'Add TCP, stdio or UART, standalone, and embeddable-library adapters around the one AVR-first VirtualBoard core rather than creating another implementation.'),
  alphaEdgeReplacement: prompt(SESSION.issueLedger, 235, 'Keep alpha application authentication and authorization disabled during exact-main edge replacement; retain explicit listener and Origin controls and only optional outbound bearer compatibility for upgrading an older host.'),
  enclosureWebControls: prompt(SESSION.issueLedger, 244, 'Expose complete live enclosure illumination settings and authoritative applied state in WebUI, hide controls when unavailable, preserve unrelated settings, and synchronize every client from pushed events.'),
...
const EXTRA_ISSUE_ORIGINAL_REQUESTS = {
  ...
  208: [PROMPT_EXCERPTS.removeAlphaSchemaBaggage],
  209: [PROMPT_EXCERPTS.compactCapabilityDiscovery],
  216: [PROMPT_EXCERPTS.uartFirstProgramming],
  217: [PROMPT_EXCERPTS.discoveryResponderCoexistence],
  221: [PROMPT_EXCERPTS.ownershipRoots],
  227: [PROMPT_EXCERPTS.virtualBoardAdapters],
  235: [PROMPT_EXCERPTS.alphaEdgeReplacement, PROMPT_EXCERPTS.authDefer],
  244: [PROMPT_EXCERPTS.enclosureWebControls],
  248: [PROMPT_EXCERPTS.deterministicBuildHelpers],
...
const PR_ORIGINAL_REQUESTS = {
  ...
  210: [PROMPT_EXCERPTS.firstRun, PROMPT_EXCERPTS.programmingEntrypoint, PROMPT_EXCERPTS.emptyBoardAcceptance],
  211: [PROMPT_EXCERPTS.firstRun, PROMPT_EXCERPTS.programmingEntrypoint, PROMPT_EXCERPTS.emptyBoardAcceptance],
  212: [PROMPT_EXCERPTS.webUi, PROMPT_EXCERPTS.webRoot],
  213: [PROMPT_EXCERPTS.buzzerFallback, PROMPT_EXCERPTS.linuxParity],
  214: [PROMPT_EXCERPTS.firstRun, PROMPT_EXCERPTS.firmwareIdentity, PROMPT_EXCERPTS.resetSafety],
  215: [PROMPT_EXCERPTS.networkDiscovery, PROMPT_EXCERPTS.discoveryMetadata],
  218: [PROMPT_EXCERPTS.uartFirstProgramming, PROMPT_EXCERPTS.programmingEntrypoint],
  219: [PROMPT_EXCERPTS.networkDiscovery, PROMPT_EXCERPTS.discoveryMetadata],
  220: [PROMPT_EXCERPTS.toastIcon, PROMPT_EXCERPTS.canonicalLogo],
  222: [PROMPT_EXCERPTS.toastIcon, PROMPT_EXCERPTS.canonicalLogo],
  223: [PROMPT_EXCERPTS.canonicalContracts, PROMPT_EXCERPTS.sharedFirmware],
  224: [PROMPT_EXCERPTS.buzzerFallback, PROMPT_EXCERPTS.linuxParity, PROMPT_EXCERPTS.externalBeepFallback],
  225: [PROMPT_EXCERPTS.latestDependencies, PROMPT_EXCERPTS.dependencyBlocker],
  226: [PROMPT_EXCERPTS.ownershipRoots, PROMPT_EXCERPTS.sharedFirmware],
  228: [PROMPT_EXCERPTS.virtualBoardAdapters, PROMPT_EXCERPTS.protocolSimulator],
  229: [PROMPT_EXCERPTS.networkDiscovery, PROMPT_EXCERPTS.discoveryMetadata],
  230: [PROMPT_EXCERPTS.sharedFirmware, PROMPT_EXCERPTS.macroPlayback],
  231: [PROMPT_EXCERPTS.linuxParity, PROMPT_EXCERPTS.stuckUpdate],
  232: [PROMPT_EXCERPTS.configurationSources, PROMPT_EXCERPTS.toolingEntrypoint],
  233: [PROMPT_EXCERPTS.sharedFirmware, PROMPT_EXCERPTS.macroPlayback],
  234: [PROMPT_EXCERPTS.linuxParity, PROMPT_EXCERPTS.stuckUpdate],
  236: [PROMPT_EXCERPTS.virtualBoardAdapters, PROMPT_EXCERPTS.sharedFirmware],
  237: [PROMPT_EXCERPTS.displaysAudio],
  238: [PROMPT_EXCERPTS.ownershipRoots, PROMPT_EXCERPTS.sharedFirmware],
  239: [PROMPT_EXCERPTS.virtualBoardAdapters, PROMPT_EXCERPTS.canonicalContracts],
  240: [PROMPT_EXCERPTS.relaySafety, PROMPT_EXCERPTS.motionDoorPolicy],
  241: [PROMPT_EXCERPTS.canonicalContracts, PROMPT_EXCERPTS.compactCapabilityDiscovery],
  242: [PROMPT_EXCERPTS.relaySafety, PROMPT_EXCERPTS.motionDoorPolicy],
  243: [PROMPT_EXCERPTS.frontPanelMirror, PROMPT_EXCERPTS.relaySafety],
  245: [PROMPT_EXCERPTS.authDefer, PROMPT_EXCERPTS.webUi],
  246: [PROMPT_EXCERPTS.ownershipRoots, PROMPT_EXCERPTS.sharedFirmware],
  247: [PROMPT_EXCERPTS.instanceCoordination, PROMPT_EXCERPTS.artifactSync, PROMPT_EXCERPTS.stuckUpdate],
  249: [PROMPT_EXCERPTS.hostEepromDefaults, PROMPT_EXCERPTS.boardAutomations],
  250: [PROMPT_EXCERPTS.hostEepromDefaults, PROMPT_EXCERPTS.boardMenus],
  251: [PROMPT_EXCERPTS.canonicalContracts, PROMPT_EXCERPTS.hostEepromDefaults],
  253: [PROMPT_EXCERPTS.tuiConsole, PROMPT_EXCERPTS.linuxParity],
  254: [PROMPT_EXCERPTS.displaysAudio, PROMPT_EXCERPTS.buzzerFallback],
  256: [PROMPT_EXCERPTS.relaySafety, PROMPT_EXCERPTS.motionDoorPolicy],
  257: [PROMPT_EXCERPTS.liveMeasurementTiming, PROMPT_EXCERPTS.peripheralPresenceUI],
  259: [PROMPT_EXCERPTS.displaysAudio, PROMPT_EXCERPTS.buzzerFallback],
  260: [PROMPT_EXCERPTS.fullRemoteTui, PROMPT_EXCERPTS.instanceCoordination, PROMPT_EXCERPTS.permissionAvailability, PROMPT_EXCERPTS.authDefer],
