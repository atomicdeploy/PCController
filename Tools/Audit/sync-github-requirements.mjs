...
const R = [
  ...
  requirement('host-automation-hotkeys-os', 7, 'Complete macros, melodies, automations, hotkeys, notifications, and guarded OS actions', 'open',
    ...
    ], 'Macros/effects/melodies, event automations, configurable global hotkeys, audited virtual-key injection, actionable Windows notifications, Running-door warning/clear, guarded Windows OS actions, DDC/CI external brightness, and native WMI laptop-panel fallback are source-tested. Cross-platform multi-display enumeration, native system volume/mute, normalized battery state, durable power-operation outcome tracking, and complete UI/API exposure remain missing. Toast delivery and shortcut registration still spawn PowerShell, and Jump List/taskbar/thumbnail plus optional external-adapter contracts are absent. Live hotkey/toast action, harmless brightness, and physical macro observations also remain open.'),
  ...
  requirement('canonical-host-artifact-packaging', 11, 'Produce one current source-identified controller artifact with verified packaging', 'open',
    ...
    ], 'Canonical product metadata drives mutable host/WebUI/resource/document titles while stable technical identifiers remain fixed. The packaged executable contains the seven-size APP icon and the host now explicitly applies that named resource to classic conhost windows instead of relying on inconsistent inherited/default icon selection. The current native Windows c-shared DLL/header build and a stable external-C ports-ABI smoke pass record compiler provenance and binary integrity. Final canonical executable/DLL packaging, exact resource/UPX/embedded-default identity, stale-shadow rejection, polished installer/repair/uninstall inventory, direct native desktop registration, and packaged console-icon observation across launch paths remain open.'),
