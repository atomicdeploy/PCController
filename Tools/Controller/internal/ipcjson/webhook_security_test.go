...
func TestInboundWebhookDropsCredentialBoundariesAndRawRequestURI(t *testing.T) {
	...
	request := httptest.NewRequest(
		http.MethodPost,
		"http://127.0.0.1:8787/api/v1/webhooks/inbound?project=alpha&access_token=query-secret&ticket=one-use-secret&signature=signed-secret",
	...
	for _, forbidden := range []string{
		bearer, "query-secret", "one-use-secret", "signed-secret",
		"body-token", "spoofed", "cookie-secret", "referrer-secret",
		"api-key-secret", "custom-secret", "access_token=", "ticket=",
	} {
	...
	if event.Text != "safe payload" || event.Source != "webhook" ||
		...
		event.Metadata["http.path"] != "/api/v1/webhooks/inbound" {
...
func TestInboundWebhookEmptyBodyFallbackContainsPathWithoutQuery(t *testing.T) {
	...
	request := httptest.NewRequest(
		http.MethodGet,
		"http://127.0.0.1:8787/api/v1/webhooks/inbound?ticket=must-never-appear",
		nil,
	)
	...
	if event.Text != "GET /api/v1/webhooks/inbound" ||
		strings.Contains(event.Text, "?") || strings.Contains(event.Text, "must-never-appear") {
		t.Fatalf("fallback text=%q", event.Text)
	}
	if _, exists := event.Metadata["query.ticket"]; exists {
		t.Fatalf("ticket metadata leaked: %#v", event.Metadata)
	}
}
