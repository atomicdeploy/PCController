...
func hostConfigCommand(options CommandOptions, args []string) (string, error) {
	const usage = "config get PATH | config set PATH VALUE; PATH includes programming.firmware_features, ui.app_title, ui.tagline, ui.appearance.*, ui.tui_console.*, or integrations.buzzer_mirror.{enabled,native_enabled,web_audio_enabled,driver_directory}"
	if len(args) < 2 {
