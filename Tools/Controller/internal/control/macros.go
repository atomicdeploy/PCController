...
// MacroRecordingState describes a PC-owned recording session. Each captured
...
func macroStepDueUS(step appconfig.MacroStep) (uint32, error) {
	if step.AtMS < 0 || (step.AtUS != 0 && step.AtMS != 0) {
		return 0, errors.New("use one nonnegative at_us or at_ms offset")
	}
	due := step.AtUS
	if step.AtUS == 0 && step.AtMS != 0 {
		if step.AtMS > 2_147_483 {
			return 0, errors.New("timing exceeds the MCU signed clock window")
		}
...
func compileMacroCommand(step appconfig.MacroStep) (byte, []byte, error) {
	...
	case "pwm-mode":
		if step.Value > uint16(native.PWMAuto) {
			return 0, nil, fmt.Errorf("PWM mode %d is outside 0..2", step.Value)
		}
		return native.OpPWMMode, []byte{byte(step.Value)}, nil
	case "relays-off":
...
func macroQueueableOpcode(opcode byte) bool {
	...
	case native.OpSetStream, native.OpSetSettings,
		native.OpBuzzer, native.OpPWMSet, native.OpPWMAllOff, native.OpPWMMode,
		...
		native.OpRFLearnRemove, native.OpRFMap, native.OpRFLearnReplace,
...
func recordedMacroStep(evidence CommandEvidence) (appconfig.MacroStep, bool) {
	...
	case native.OpPWMAllOff:
		step.Kind = "pwm-off"
	case native.OpPWMMode:
		if len(payload) != 1 {
			return step, false
		}
		step.Kind, step.Value = "pwm-mode", uint16(payload[0])
