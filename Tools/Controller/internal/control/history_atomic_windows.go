...
var historyMoveFileEx = syscall.NewLazyDLL("kernel32.dll").NewProc("MoveFileExW")
...
func writeHistoryFileAtomic(path string, content []byte, mode os.FileMode) error {
	...
	result, _, callErr := historyMoveFileEx.Call(
	...
	return errors.New("MoveFileExW rejected atomic measurement-history replacement")
}
