...
func mirrorTestConfig(t *testing.T) Config {
	...
	config.Paths = Paths{
		...
		UnattendedShim:   filepath.Join(root, "opt", "pccontroller", "libexec", "unattended-upgrade"),
		UnattendedDropIn: filepath.Join(root, "etc", "systemd", "apt-daily-upgrade.service.d", "50-pccontroller-origin-cache.conf"),
		APTDailyDropIn:   filepath.Join(root, "etc", "systemd", "apt-daily.service.d", "50-pccontroller-proxy.conf"),
