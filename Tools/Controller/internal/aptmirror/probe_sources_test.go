...
func TestHTTPProberValidatesSignedReferencedComponentReleases(t *testing.T) {
	...
	fixture := signedFixture(suite, "resolute", "amd64", now.Add(-time.Hour), now.Add(24*time.Hour).Format(time.RFC1123), releases)
	...
	// binary-amd64 Release object which the endpoint does not actually serve.
	delete(releases, "universe/binary-amd64/Release")
	result = prober.Probe(context.Background(), config, Candidate{ID: "liara", Role: RoleDomestic, URI: server.URL + "/repository/ubuntu/"}, suite)
...
func TestInstallDryRunAndGeneratedTopologyDoNotMutate(t *testing.T) {
	...
	if strings.Count(source, "mirror+file:") != 1 || !strings.Contains(source, "Architectures: amd64") {
