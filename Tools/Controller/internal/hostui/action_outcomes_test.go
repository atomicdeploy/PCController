...
func TestActionCapabilitiesAreBoundedAndCanonical(t *testing.T) {
	...
	if value, err := ActionCapabilities("future.optional"); err != nil || value != "future.optional" {
		t.Fatalf("bounded optional capability was lost: %q %v", value, err)
	}
	if _, err := ActionCapabilities("invalid capability"); err == nil {
	...
	if _, err := registry.Upsert(AppInstance{
		...
		Values: map[string]string{ActionCapabilitiesKey: "app.title,future.optional"},
	}); err != nil {
		t.Fatalf("optional future capability rejected the whole instance: %v", err)
	}
}
...
func TestActionCoordinatorRejectsAckAtExactDeadline(t *testing.T) {
	...
	coordinator := NewActionCoordinator(registry, func(action AppAction) error { delivery = action; return nil })
	defer coordinator.Close()
	now := time.Now()
	coordinator.now = func() time.Time { return now }
	operation, err := coordinator.Submit(AppAction{Kind: "app.title", Value: "Title", Target: "web:deadline"}, MaximumActionTimeout)
	if err != nil {
		t.Fatal(err)
	}
	now = operation.ExpiresAt
	_, err = coordinator.Ack(ActionAck{OperationID: operation.OperationID, DeliveryID: delivery.Metadata[ActionDeliveryIDKey], InstanceID: "web:deadline", State: ActionStateApplied})
	if err == nil {
		t.Fatal("late ACK beat an un-fired timeout timer")
	}
	current, err := coordinator.Outcome(operation.OperationID)
	if err != nil || current.State != ActionStateTimeout {
