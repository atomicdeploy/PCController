...
// client. Unknown optional capabilities survive discovery, but cannot become
// executable actions until this coordinator implements them.
func ActionCapabilities(values ...string) (string, error) {
	if len(values) > 32 {
		return "", errors.New("app action capabilities exceed 32 entries")
	}
	seen := make(map[string]struct{}, len(values))
	for _, raw := range values {
		value := strings.ToLower(strings.TrimSpace(raw))
		if !instanceValuePattern.MatchString(value) {
			return "", fmt.Errorf("invalid app action capability %q", raw)
		}
		seen[value] = struct{}{}
	}
...
func (coordinator *ActionCoordinator) Ack(value ActionAck) (ActionOperation, error) {
	...
	if current.State == ActionStateQueued && !now.Before(record.operation.ExpiresAt) {
		coordinator.mu.Unlock()
		coordinator.timeout(value.OperationID)
		return ActionOperation{}, errors.New("app action acknowledgement arrived after delivery expired")
	}
