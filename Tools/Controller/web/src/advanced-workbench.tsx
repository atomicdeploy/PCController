    ...
    <section className="advanced-surface" aria-labelledby="advanced-workbench-title">
      ...
      <div className="advanced-grid">
        ...
        >
          <div className="advanced-actions">
            ...
            <Button icon={Activity} disabled={!online} busy={busy === 'status'} onClick={() => void run('status')}>{copy('Live status', 'وضعیت زنده')}</Button>
            <Button busy={busy === 'program-state'} onClick={() => void run('program-state')}>{copy('Program state', 'حالت برنامه')}</Button>
          </div>
          <div className="advanced-fields advanced-fields--split">
            <TextField label={copy('Serial port · blank = authenticated auto-detect', 'درگاه سریال · خالی = کشف امن خودکار')} value={port} dir="ltr" spellCheck={false} onChange={(event) => setPort(event.target.value)} />
            <div className="advanced-inline-actions">
              <Button tone="primary" busy={busy === serialOpen} onClick={() => void run(serialOpen)}>{copy('Open', 'اتصال')}</Button>
              <Button busy={busy === 'reconnect'} onClick={() => void run('reconnect')}>{copy('Reconnect', 'اتصال مجدد')}</Button>
              <Button busy={busy === 'close'} onClick={() => void run('close')}>{copy('Close & pause', 'بستن و توقف')}</Button>
            </div>
          </div>
          <div className="advanced-control-row">
            ...
            />
            <RangeField label={copy('Period', 'دوره')} value={streamPeriod} min={100} max={5000} step={100} unit="ms" disabled={!online || !streamEnabled} onChange={setStreamPeriod} />
            <Button disabled={!online} busy={busy === streamCommand} onClick={() => void run(streamCommand)}>{copy('Apply stream', 'اعمال جریان')}</Button>
          </div>
          <div className="advanced-fields advanced-fields--program">
            <div className="advanced-field">
              ...
              <Segmented value={programMode} label={copy('Program mode', 'حالت برنامه')} options={[{ value: 'idle', label: 'Idle' }, { value: 'running', label: 'Running' }]} onChange={(value) => { setProgramMode(value); setProgramModeTouched(true) }} />
            </div>
        ...
        >
          <div className="advanced-actions">
            <Button disabled={!online} busy={busy === 'temp'} onClick={() => void run('temp')}>{copy('Read values', 'خواندن دما')}</Button>
            <Button disabled={!online} busy={busy === 'temp list'} onClick={() => void run('temp list')}>{copy('List identities', 'فهرست شناسه‌ها')}</Button>
        ...
        >
          <div className="advanced-actions">
            <Button disabled={!online} busy={busy === 'settings'} onClick={() => void run('settings')}>{copy('EEPROM status', 'وضعیت EEPROM')}</Button>
            <Button disabled={!online} busy={busy === 'silent status'} onClick={() => void run('silent status')}>{copy('Silent status', 'وضعیت بی‌صدا')}</Button>
          </div>
          ...
          <div className="advanced-actions">
            <Button disabled={!online} busy={busy === `silent ${silent ? 'on' : 'off'}`} onClick={() => void run(`silent ${silent ? 'on' : 'off'}`)}>{copy('Apply silent', 'اعمال بی‌صدا')}</Button>
            <Button disabled={!online} busy={busy === `settings audio door ${doorAudio ? 'on' : 'off'}`} onClick={() => void run(`settings audio door ${doorAudio ? 'on' : 'off'}`)}>{copy('Apply door cues', 'اعمال صدای در')}</Button>
            <Button disabled={!online} busy={busy === `settings audio relay ${relayAudio ? 'on' : 'off'}`} onClick={() => void run(`settings audio relay ${relayAudio ? 'on' : 'off'}`)}>{copy('Apply relay cues', 'اعمال صدای رله')}</Button>
          </div>
        </AdvancedPanel>
        ...
        <AdvancedPanel
          ...
          detail={copy('Firmware page IDs and PC-owned menu overlays remain separately inspectable.', 'شناسه صفحات میان‌افزار و منوهای میزبان به‌صورت مستقل قابل بررسی هستند.')}
        >
          <div className="advanced-actions">
            <Button disabled={!online} busy={busy === 'menu list'} onClick={() => void run('menu list')}>{copy('Firmware catalog', 'کاتالوگ میان‌افزار')}</Button>
            <Button disabled={!online} busy={busy === 'menu current'} onClick={() => void run('menu current')}>{copy('Current page', 'صفحه فعلی')}</Button>
            <Button disabled={!online} busy={busy === 'menu layout'} onClick={() => void run('menu layout')}>{copy('Stored layout', 'چیدمان ذخیره‌شده')}</Button>
            <Button busy={busy === 'host-menu list'} onClick={() => void run('host-menu list')}>{copy('Host catalog', 'کاتالوگ میزبان')}</Button>
            <Button busy={busy === 'host-menu directory'} onClick={() => void run('host-menu directory')}>{copy('Host directory', 'دایرکتوری میزبان')}</Button>
            <Button busy={busy === 'host-menu status'} onClick={() => void run('host-menu status')}>{copy('Overlay status', 'وضعیت لایه میزبان')}</Button>
          </div>
          ...
          <div className="advanced-fields advanced-fields--split">
            <TextField label={copy('Firmware page ID or key', 'شناسه یا کلید صفحه میان‌افزار')} value={menuPage} dir="ltr" spellCheck={false} onChange={(event) => setMenuPage(event.target.value)} />
            <Button disabled={!online || !menuPage.trim()} busy={busy === `menu page ${menuPage.trim()}`} onClick={() => void run(`menu page ${quoteArgument(menuPage.trim())}`)}>{copy('Go to page', 'رفتن به صفحه')}</Button>
            <TextField label={copy('Host menu ID · optional', 'شناسه منوی میزبان · اختیاری')} value={hostMenuID} dir="ltr" spellCheck={false} onChange={(event) => setHostMenuID(event.target.value)} />
            <div className="advanced-inline-actions">
              <Button disabled={!online} busy={busy === `host-menu open${hostMenuID.trim() ? ` ${quoteArgument(hostMenuID.trim())}` : ''}`} onClick={() => void run(`host-menu open${hostMenuID.trim() ? ` ${quoteArgument(hostMenuID.trim())}` : ''}`)}>{copy('Open overlay', 'باز کردن لایه')}</Button>
              <Button disabled={!online} busy={busy === 'host-menu refresh'} onClick={() => void run('host-menu refresh')}>{copy('Refresh', 'تازه‌سازی')}</Button>
              <Button busy={busy === 'host-menu close'} onClick={() => void run('host-menu close')}>{copy('Close', 'بستن')}</Button>
            </div>
          </div>
        ...
        >
          ...
          <Button tone="primary" disabled={!online} busy={busy === `strip pixel ${pixel} ${pixelRed} ${pixelGreen} ${pixelBlue} ${pixelBrightness}`} onClick={() => void run(`strip pixel ${pixel} ${pixelRed} ${pixelGreen} ${pixelBlue} ${pixelBrightness}`)}>{copy('Apply pixel', 'اعمال پیکسل')}</Button>
          ...
          <div className="advanced-actions">
            <Button busy={busy === 'rgb effect list'} onClick={() => void run('rgb effect list')}>{copy('List effects', 'فهرست افکت‌ها')}</Button>
            <Button busy={busy === 'rgb effect status'} onClick={() => void run('rgb effect status')}>{copy('Effect status', 'وضعیت افکت')}</Button>
            <Button disabled={!online || !effectName.trim()} busy={busy === `rgb effect play ${quoteArgument(effectName.trim())}`} onClick={() => void run(`rgb effect play ${quoteArgument(effectName.trim())}`)}>{copy('Play effect', 'اجرای افکت')}</Button>
        ...
        >
          <div className="advanced-actions">
            ...
            <Button disabled={!online} busy={busy === 'rf list'} onClick={() => void run('rf list')}>{copy('Inspect learned entries', 'بررسی ورودی‌های آموخته‌شده')}</Button>
          </div>
          ...
          <div className="advanced-actions">
            <Button tone="danger" disabled={!rfID.trim()} onClick={() => prepare(`rf remove ${rfID.trim()}`, copy('This permanently removes one learned RF entry from the board.', 'این فرمان یک ورودی RF آموخته‌شده را برای همیشه از برد حذف می‌کند.'), 'danger', true)}>{copy('Prepare removal', 'آماده‌سازی حذف')}</Button>
          </div>
          <div className="advanced-field">
        ...
        >
          <div className="advanced-actions">
            <Button busy={busy === 'macro list'} onClick={() => void run('macro list')}>{copy('List', 'فهرست')}</Button>
            <Button busy={busy === 'macro status'} onClick={() => void run('macro status')}>{copy('Playback status', 'وضعیت اجرا')}</Button>
            <Button busy={busy === 'macro record status'} onClick={() => void run('macro record status')}>{copy('Recording status', 'وضعیت ضبط')}</Button>
          </div>
          <div className="advanced-fields advanced-fields--split">
            <TextField label={copy('Macro name or ID', 'نام یا شناسه ماکرو')} value={macroRef} dir="ltr" spellCheck={false} onChange={(event) => setMacroRef(event.target.value)} />
            <Button disabled={!macroRef.trim()} busy={busy === `macro show ${quoteArgument(macroRef.trim())}`} onClick={() => void run(`macro show ${quoteArgument(macroRef.trim())}`)}>{copy('Inspect exact steps', 'بررسی گام‌های دقیق')}</Button>
          </div>
          <div className="advanced-fields advanced-fields--record">
            ...
            <Button tone="primary" disabled={!online || !macroName.trim() || (!!macroColor.trim() && !macroCategory.trim())} busy={busy === macroRecordCommand} onClick={() => void run(macroRecordCommand)}>{copy('Start recording', 'شروع ضبط')}</Button>
          </div>
          <div className="advanced-actions">
            <Button busy={busy === 'macro record save'} onClick={() => void run('macro record save')}>{copy('Save recording', 'ذخیره ضبط')}</Button>
            <Button busy={busy === 'macro record discard'} onClick={() => void run('macro record discard')}>{copy('Discard recording', 'حذف ضبط')}</Button>
            <Button disabled={!online} busy={busy === 'macro cancel'} onClick={() => void run('macro cancel')}>{copy('Cancel safely', 'لغو امن')}</Button>
            <Button tone="danger" disabled={!online} onClick={() => prepare('macro cancel keep', copy('Cancelling with keep deliberately leaves current physical outputs unchanged.', 'لغو با حفظ خروجی، وضعیت فعلی خروجی‌های فیزیکی را عمداً نگه می‌دارد.'), 'danger', true)}>{copy('Prepare cancel + keep', 'آماده‌سازی لغو با حفظ خروجی')}</Button>
          </div>
        </AdvancedPanel>
        ...
        >
          <div className="advanced-actions">
            <Button disabled={!online} busy={busy === 'i2c scan'} onClick={() => void run('i2c scan')}>{copy('Scan bus', 'پویش گذرگاه')}</Button>
            <Button busy={busy === 'i2c lcd status'} onClick={() => void run('i2c lcd status')}>{copy('LCD status', 'وضعیت LCD')}</Button>
            <Button disabled={!online} busy={busy === 'i2c lcd rescan'} onClick={() => void run('i2c lcd rescan')}>{copy('Rescan LCD', 'پویش دوباره LCD')}</Button>
            <Button disabled={!online} busy={busy === 'i2c release'} onClick={() => void run('i2c release')}>{copy('Release lease', 'آزادسازی دسترسی')}</Button>
          </div>
        ...
        >
          <div className="advanced-actions">
            <Button busy={busy === 'keyboard status'} onClick={() => void run('keyboard status')}>{copy('Hook status', 'وضعیت هوک')}</Button>
            <Button busy={busy === 'keyboard list'} onClick={() => void run('keyboard list')}>{copy('Binding catalog', 'فهرست نگاشت‌ها')}</Button>
            <Button onClick={() => prepare('keyboard enable', copy('Enabling the global hook activates the configured bindings; review them first with Keyboard list.', 'فعال‌سازی هوک سراسری، نگاشت‌های پیکربندی‌شده را فعال می‌کند؛ ابتدا فهرست صفحه‌کلید را بازبینی کنید.'), 'caution')}>{copy('Review enable', 'بازبینی فعال‌سازی')}</Button>
            <Button busy={busy === 'keyboard disable'} onClick={() => void run('keyboard disable')}>{copy('Disable & release', 'غیرفعال و آزادسازی')}</Button>
        ...
        >
          <div className="advanced-actions">
            <Button busy={busy === 'os status'} onClick={() => void run('os status')}>{copy('Host status', 'وضعیت میزبان')}</Button>
            <Button busy={busy === 'os policy'} onClick={() => void run('os policy')}>{copy('Action policy', 'سیاست فرمان‌ها')}</Button>
            <Button busy={busy === 'os brightness get'} onClick={() => void run('os brightness get')}>{copy('Read brightness', 'خواندن روشنایی')}</Button>
          </div>
          <div className="advanced-fields advanced-fields--split">
        ...
        >
          <form className="advanced-form" onSubmit={sendMessage}>
            <div className="advanced-field">
              ...
              <Segmented value={messageTarget} label={copy('Message target', 'مقصد پیام')} options={[
                { value: 'host', label: copy('Event stream', 'جریان رویداد') },
                { value: 'lcd', label: copy('Physical LCD', 'LCD فیزیکی') },
              ]} onChange={setMessageTarget} />
            </div>
        ...
        >
          <div className="advanced-actions">
            <Button icon={Cpu} disabled={!online} busy={busy === 'hello'} onClick={() => void run('hello')}>{copy('Firmware identity', 'شناسه میان‌افزار')}</Button>
            ...
            <Button busy={busy === 'toolchain core-info'} onClick={() => void run('toolchain core-info')}>{copy('Core info', 'اطلاعات هسته')}</Button>
          </div>
          <TextField label={copy('Backup directory or read output file', 'پوشه پشتیبان یا فایل خروجی خواندن')} value={bootPath} dir="ltr" spellCheck={false} onChange={(event) => setBootPath(event.target.value)} />
          <div className="advanced-actions">
          ...
          <p className="advanced-note advanced-note--safe">{copy('This deck intentionally exposes no flash, bootloader-write, raw serial-write, reset, or toolchain-install shortcut.', 'این میز عمداً هیچ میان‌بر فلش، نوشتن بوت‌لودر، نوشتن خام سریال، ریست یا نصب زنجیره‌ابزار ندارد.')}</p>
        </AdvancedPanel>
      </div>
      ...
      <section id="advanced-command-review" className={`advanced-review advanced-review--${review.risk}`} aria-live="polite">
        ...
        <div className="advanced-review__actions">
          ...
          <Button disabled={!review.command || reviewBusy} onClick={() => { setReview({ command: '', reason: '', risk: 'normal', needsDevice: false }); setDangerArm('') }}>{copy('Clear', 'پاک‌کردن')}</Button>
