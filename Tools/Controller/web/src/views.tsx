...
export function SettingsView({ appTitle, snapshot, events, locale, t, command, appearance, onAppTitle, boardSettingsReadState, uiConfig, onBuzzerPath, openAppPreferences }: SharedViewProps & { appearance: Appearance; onAppearance: (value: Appearance) => void; /** Accepted but intentionally ignored while alpha auth is disabled. */ token?: string; onToken?: (value: string) => void; onAppTitle: (value: string) => Promise<string>; uiConfig: UIConfig | null; onBuzzerPath: (value: BuzzerPath) => Promise<void> }) {
  const copy = (english: string, persian: string) => locale === 'fa' ? persian : english
