...
board is on its Status page requests the default hosted menu. While captured:
...
- K4 enters a submenu or selects an enabled action. On a read-only, disabled,
  or PC-only text item it is a true no-op: no read callback, value, or revision
...
`select`, `submenu`, and `action`. Editing an active label/title/content updates
...
```json
{
  "host_menus": {
    "default_menu": "host",
    "request_gesture": "status-hold-k4",
...
Built-in read/write actions cover host/device status, IP/API state, application
title, polling period, LCD prompt mirroring, DTR reset-on-reconnect, and primary
monitor brightness. A `shell:` or `command:` action routes through the existing
...
and confirmation token. The DDC/CI brightness item reads 0..100 and changes in
steps of five; an internal panel, adapter, or display driver without DDC/CI
returns a clean unsupported result.

## Protocol contract

- HELLO bit 19 authorizes host capture.
- HELLO bit 23 authorizes the board-owned EEPROM menu visibility/order record.
  Stable built-in IDs remain `0..14`; the compact schema-2 payload is 12 bytes:
  `[2, 15, visible-mask LE, eight packed rank nibbles]` with an `0xF` unused
  high tail nibble. Hosts also decode the older expanded schema-1 artifact.
...
capability-19 capture path. On that legacy path the host approximates
...
Firmware no longer implements the old MCU macro opcodes `0x39..0x3B`. A hosted
macro recorder/player must schedule direct relay/PWM commands and publish its
name, ID, elapsed time, duration, step, and lifecycle through this hosted-menu
and host-event seam; it must not revive or assume those removed opcodes.
...
for completion. Legacy firmware never has raw bit 12 reinterpreted; it receives
