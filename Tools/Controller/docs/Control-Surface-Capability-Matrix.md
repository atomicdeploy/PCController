...
| Buzzer, silence, and melodies | `beep`, `silent`, `melody` | `PlayTone`, `StartMelody`, configured melody scheduler, stop/status methods | Generic command | output scheduler state and buzzer-busy telemetry when advertised | query: `read`; play/stop: `board_commands`; create/delete: `host_configuration` |
...
| Typed text, actionable notifications, and LCD messages | message API rather than free-form shell text; TUI `Ctrl+A` runs only the selected retained action | `SendTextMessage`, delivery/action outcome helpers | `controller.message.send`, `.delivery`, `.action`; `/api/messages`; webhooks/Socket.IO | source-tagged message plus correlated per-surface delivery/action outcomes; optional LCD presentation | `messages` |
