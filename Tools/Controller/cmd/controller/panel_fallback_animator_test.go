...
func TestPanelFallbackAnimatorReloadCancelAndDisconnect(t *testing.T) {
	var mu sync.Mutex
	frames := make([]string, 0, 8)
	disconnected := false
	animator := newPanelFallbackAnimator(func(snapshot hostmenu.Snapshot) error {
		mu.Lock()
		defer mu.Unlock()
		if disconnected {
	...
	waitForFallbackFrames(t, &mu, &frames, 2)
	...
	mu.Unlock()
	waitForFallbackFrames(t, &mu, &frames, reloadIndex+2)
	mu.Lock()
...
func TestPanelFallbackVisualVariants(t *testing.T) {
	panel := hostmenu.Panel{Segments: "EDIT", EditVisual: "edit-dim"}
	if got := panelFallbackSegments(panel, false); got != "----" {
	...
	if !panelFallbackAnimated(panel) {
		t.Fatal("blink panel was not recognized as animated")
	}
}

func waitForFallbackFrames(t *testing.T, mu *sync.Mutex, frames *[]string, count int) {
	t.Helper()
